...
void main() {
    ...
    float rimWidth = min(max_dist_norm * 0.5, 60.0);
    float edge_gradient = 1.0 - clamp(distToEdge / rimWidth, 0.0, 1.0);
    ...
    vec2 dir = pos / max(length(pos), 1e-4);
    vec2 distortedTexCoord = TexCoord + dir * fresnel * DistortStrength;

    vec4 texColor = texture2D(Sampler0, distortedTexCoord) * FragColor;
