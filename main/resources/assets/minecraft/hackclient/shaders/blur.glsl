...
{
    return texture2D(u_diffuse_sampler, uv);
}

vec4 bgStrips(vec2 uv)
{
    // Используем фиксированное значение для полос
...
void main()
{
    ...
    if (u_direction.x != 0.0) {
        ...
        // Преобразуем UV в пиксельные координаты используя u_texel_size
        vec2 fragCoord = uv / u_texel_size;
        vec2 center = vec2(0.5, 0.5) / u_texel_size;
        ...
        // Преобразуем смещение из пикселей обратно в UV координаты
        vec2 coord1_px = fragCoord + refract_vec.xy * refract_length;
        vec2 coord1_uv = coord1_px * u_texel_size;
        vec4 refract_color = bg(coord1_uv);
