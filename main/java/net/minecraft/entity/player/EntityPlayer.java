...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public void onUpdate() {
        Phase phase = Modules.getInstance().getModule(Phase.class);
        Test test = Modules.getInstance().getModule(Test.class);
        Test2 test2 = Modules.getInstance().getModule(Test2.class);

        this.noClip = phase.isToggled() || this.isSpectator() || test.shouldNoclip() || test2.isVonyat();
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem()) {
            if (!targetEntity.hitByEntity(this)) {
                AttackEvent event = AttackEvent.getInstance();
                event.setHittingEntity(targetEntity);
                event.setCancelSprint(true);
                event.call();
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        if (i > 0) {
                            ...
                            HitSlowDownEvent hitSlowDownEvent = HitSlowDownEvent.getInstance();
                            hitSlowDownEvent.setSlowDown(0.6);
