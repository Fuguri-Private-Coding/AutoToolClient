...
public class AntiFireball extends Module {
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof TickEvent) {
            if (target != null && (target.hurtResistantTime > 0 || target.isDead || !mc.theWorld.getLoadedEntityList().contains(target) || DistanceUtils.getDistance(target) > distance.getValue())) target = null;

            for (Entity target : mc.theWorld.loadedEntityList) {
                if (!(target instanceof EntityFireball entityFireball) || entityFireball.shootingEntity == mc.thePlayer || DistanceUtils.getDistance(target) > distance.getValue()) {
                    continue;
                }
