    ...
    public void onEvent(Event event) {
        if (event instanceof Render2DEvent && modes.get("Box")) {
            for (Entity player : mc.theWorld.loadedEntityList) {
                ...
                AxisAlignedBB bb = player.getEntityBoundingBox().offset(pos.xCoord - player.posX, pos.yCoord - player.posY, pos.zCoord - player.posZ).expand(0.05);
                ...
                Color color = boxColor.getFadedColor();
        ...
        if (event instanceof Render3DEvent && modes.get("HitBox")) {
            ...
            for (Entity player : mc.theWorld.loadedEntityList) {
                if (!shouldContinueRender(player)) {
                    Vec3 pos = RenderUtils.getAbsoluteSmoothPos(player.getLastPositionVector(), player.getPositionVector()).subtract(RenderManager.getRenderPosition());

                    AxisAlignedBB bb = player.getEntityBoundingBox().offset(pos.xCoord - player.posX, pos.yCoord - player.posY, pos.zCoord - player.posZ);

                    Color color = hitBoxColor.getFadedColor();
