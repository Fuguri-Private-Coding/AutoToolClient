package fuguriprivatecoding.autotoolrecode.module.impl.move;

import fuguriprivatecoding.autotoolrecode.event.Event;
import fuguriprivatecoding.autotoolrecode.event.EventTarget;
import fuguriprivatecoding.autotoolrecode.event.events.MoveButtonEvent;
import fuguriprivatecoding.autotoolrecode.event.events.PacketEvent;
import fuguriprivatecoding.autotoolrecode.event.events.TickEvent;
import fuguriprivatecoding.autotoolrecode.module.Category;
import fuguriprivatecoding.autotoolrecode.module.Module;
import fuguriprivatecoding.autotoolrecode.module.ModuleInfo;
import fuguriprivatecoding.autotoolrecode.utils.client.ClientUtils;
import net.minecraft.block.*;
import net.minecraft.block.state.IBlockState;
import net.minecraft.network.play.client.C03PacketPlayer;
import net.minecraft.network.play.server.S08PacketPlayerPosLook;
import net.minecraft.util.BlockPos;

@ModuleInfo(name = "VClip", category = Category.MOVE, description = "Позволяет телепортироватся вверх.")
public class VClip extends Module {

    private int tickCounter = 0;
    private long startTime;
    private int remainingTime = 6000;

    @Override
    public void onEnable() {
        remainingTime = 6000;
        if (!isNearBars()) {
            this.startTime = System.currentTimeMillis();
            this.remainingTime = 6000;
        }
    }

    @EventTarget
    public void onEvent(Event event) {
        if (!isNearBars() && event instanceof PacketEvent packetEvent && (packetEvent.getPacket() instanceof C03PacketPlayer.C06PacketPlayerPosLook || packetEvent.getPacket() instanceof S08PacketPlayerPosLook)) packetEvent.setCanceled(true);
        if (!isNearBars() && event instanceof MoveButtonEvent moveButtonEvent) {
            moveButtonEvent.setJump(false);
            moveButtonEvent.setForward(false);
            moveButtonEvent.setRight(false);
            moveButtonEvent.setLeft(false);
            moveButtonEvent.setBack(false);
        }
        if (event instanceof TickEvent) {
            if (isNearBars()) {
                setToggled(false);
                ClientUtils.chatLog("Встань в неполный блок!");
            } else {
                long currentTime = System.currentTimeMillis();
                long elapsedTime = currentTime - startTime;
                remainingTime = Math.max(0, 6000 - (int) elapsedTime);
                if (mc.thePlayer != null) {
                    if (isToggled() && isNearBars()) {
                        setToggled(false);
                        ClientUtils.chatLog("Встань в неполный блок!");
                    } else {
                        if (remainingTime <= 0) {
                            setToggled(false);
                        } else {
                            for (int k = 0; k < 9; ++k) {
                                ++tickCounter;
                                if (tickCounter == 1) {
                                    for (int i = 0; i < 9; ++i) {
                                        sendPositionPacket(mc.thePlayer.posX, mc.thePlayer.posY + i, mc.thePlayer.posZ);
                                    }
                                } else if (tickCounter == 2) {
                                    sendPositionPacket(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ);
                                } else if (tickCounter >= 3) {
                                    tickCounter = 0;
                                }
                            }
                        }
                    }
                }
            }
        }
    }

    private boolean isNearBars() {
        BlockPos playerPos = new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ);
        for (int x = -1; x <= 1; ++x) {
            for (int y = -1; y <= 1; ++y) {
                for (int z = -1; z <= 1; ++z) {
                    BlockPos checkPos = playerPos.add(x, y, z);
                    Block block = mc.theWorld.getBlockState(checkPos).getBlock();
                    if (block instanceof BlockPane || block instanceof BlockFence || block instanceof BlockWall || block instanceof BlockDoor || block instanceof BlockLadder || block instanceof BlockVine || block instanceof BlockReed || block instanceof BlockSign || block instanceof BlockWeb || block instanceof BlockFire || isValidTrapdoor(checkPos)) {
                        return false;
                    }
                }
            }
        }
        return true;
    }

    private boolean isValidTrapdoor(BlockPos pos) {
        IBlockState state = mc.theWorld.getBlockState(pos);
        if (!(state.getBlock() instanceof BlockTrapDoor)) {
            return false;
        }
        return !state.getValue(BlockTrapDoor.OPEN) && state.getValue(BlockTrapDoor.HALF) == BlockTrapDoor.DoorHalf.BOTTOM;
    }

    private void sendPositionPacket(double x, double y, double z) {
        mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(x, y + 15.0F, z, true));
    }
}
