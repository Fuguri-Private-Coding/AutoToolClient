...
public class AltScreen extends GuiScreen {
    ...
    private final Scroll scroll = new Scroll(30);
    private int scrollTotal;

    private Account selectedAccount, lastClickedAccount;
    private long lastClickedAccountTime;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        BackgroundUtils.run();
        Accounts.render(10, 10);
    }

    @Override
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        Accounts.click(10, 10, mouseX, mouseY, mouseButton);
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        Accounts.key(typedChar, keyCode);
    }
...
//            select.setEnd(isSelected(account) || account.isCurrentUsing());
...
//            fontRenderer.drawString(account.getIgn() + " " + account.getType(), accountX + 5, accountY + accountHeight / 2f - 2.5f, textColor);
//
...
//                    boolean accountExists = accounts.stream().anyMatch(acc -> acc.getIgn().equals(accountName));
//
//                    if (!accountExists) {
...
//                        isFinished.set(true);
//                    } else {
//                        addAccount(new Account(account.getIgn(), account.getToken(), account.getUuid()));
//                        selectedAccount = account;
//                        isFinished.set(true);
//                    }
//                });
...
//        selectedAccount = account;
//        updateStatus("Successful added account: " + account.getIgn() + ".");
//    }
//
...
//        selectedAccount = null;
//        updateStatus("Successful removed account: " + account.getIgn() + ".");
//    }
//
//    private void loginOffline(Account account) {
//        mc.getSession().setUsername(account.getIgn());
//        mc.getSession().setSessionType(Session.Type.LEGACY);
//        updateStatus("Successful login account: " + account.getIgn() + ".");
//    }
//
//    private void loginMicrosoft(Account account) {
//        new Thread(() -> {
//            try {
//                Map.Entry<String, String> authRefreshTokens = Auth.refreshToken(account.getToken());
...
//
//                mc.setSession(new Session(account.getIgn(),
//                    account.getUuid(),
//                    accessToken, "msa"));
//                selectedAccount = account;
