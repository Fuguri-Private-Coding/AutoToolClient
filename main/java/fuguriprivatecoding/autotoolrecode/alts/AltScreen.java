...
import fuguriprivatecoding.autotoolrecode.alts.microsoft.Auth;
import fuguriprivatecoding.autotoolrecode.alts.microsoft.MicrosoftAuthCallback;
...
public class AltScreen extends GuiScreen {
    ...
    private void loginMicrosoft(Account account, String refreshToken) {
        new Thread(() -> {
            try {
                if (account == null) {
                    Map.Entry<UUID, String> accountInfo = Auth.getProfile(refreshToken);
                    Account accounts = new Account(accountInfo.getValue(), refreshToken, accountInfo.getKey().toString());

                    mc.setSession(new Session(accounts.getName(),
                        accounts.getUuid(),
                        refreshToken, "msa"));

                    mc.getSession().setSessionType(Session.Type.MCTOKEN);
                    updateStatus("Successful login account: " + accounts.getName() + ".");
                } else {
